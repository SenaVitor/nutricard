...
describe('Food Router', () => {
  ...
  test('Should return 400 if no query is provided', async () => {
    ...
    expect(httpResponse.body.error).toBe(new MissingParamError('query').message);
  });
  ...
  test('Should return 400 if no numberOfResults is provided', async () => {
    ...
    const httpResponse = await sut.route(httpRequest);
    expect(httpResponse.statusCode).toBe(400);
    expect(httpResponse.body.error).toBe(new MissingParamError('numberOfResults').message);
  });
